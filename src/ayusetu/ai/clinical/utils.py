...
def parse_onset(text: str) -> Optional[Tuple[str, float]]:
    ...
    patterns = [
        ...
        (re.compile(r"\b(parson se|parso se|parson|parso|परसों से|परसों|day before yesterday)\b", re.I | re.U), "2 days ago", 0.90),
        (re.compile(r"\b(kal se|कल से|yesterday|kal|कल)\b", re.I | re.U), "yesterday", 0.90),
        (re.compile(r"\b(aaj subah se|subah se|आज सुबह से|सुबह से|this morning|aaj subah|aaj savere|आज सुबह)\b", re.I | re.U), "this morning", 0.90),
    ...
    # 2. Relative onset with duration + ago/pehle/back/se (e.g. "3 days ago", "3 days back", "3 din se", "teen din pehle", "तीन दिन पहले")
    ...
    rel_ago_pattern = re.compile(
        ...
        r"(?:पहले|pahle|pehle|ago|back|se|से)\b",
        re.I | re.U,
    )
    ...
    # 3. Temporal phrases indicating start/onset: "last N days", "from N days", "for N days", "since N days", "over the last N days", "in the last N days", "pichle N din"
    since_last_pattern = re.compile(
        r"\b(?:since|from\s+last|from|for\s+the\s+past|for\s+last|for|over\s+the\s+last|in\s+the\s+last|last|pichle|पिछले)\s+"
        r"(?:(\d+|" + num_keys + r")\s+)?"
...
def parse_location(text: str) -> Optional[Tuple[str, float]]:
    ...
    locations = [
        ...
        # Back
        (re.compile(
            r"(?<!\bdays\s)(?<!\bday\s)(?<!\bhours\s)(?<!\bhour\s)(?<!\bweeks\s)(?<!\bweek\s)(?<!\bmonths\s)(?<!\bmonth\s)(?<!\byears\s)(?<!\byear\s)(?<!\btime\s)\bback\b|\b(kamar|कमर|spine|peeth|पीठ|lower\s+back|upper\s+back|backache|back\s+pain)\b",
            re.I | re.U), "back", 0.88),
        # Throat / neck
        (re.compile(
